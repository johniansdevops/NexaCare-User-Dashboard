...
export default function PatientRecordsChat() {
  ...
  const getDocumentColor = (type: string) => {
    switch (type) {
      case 'lab_result': return 'text-green-600';
      case 'imaging': return 'text-blue-600';
      case 'prescription': return 'text-purple-600';
      case 'visit_note': return 'text-orange-600';
      case 'insurance': return 'text-gray-600';
  ...
  return (
    <div className="p-6 space-y-8 bg-white min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-slide-up">
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-2">AI Health Records Chat</h1>
          <p className="text-lg text-gray-600">Ask questions about your medical documents and get AI-powered insights</p>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm p-6 animate-slide-up">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-green-600 font-mono">AI Online</span>
            </div>
          </div>
                      ...
                      {message.sourceReferences && message.sourceReferences.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-gray-200">
                          ...
                          {message.sourceReferences.map((ref, index) => (
                            <div key={index} className="text-xs p-2 bg-gray-100 rounded-lg mb-1">
                              <div className="flex items-center justify-between">
                                <span className="text-blue-600 font-medium">
                                  {documents.find(d => d.id === ref.documentId)?.name}
                                </span>
                                <span className="text-green-600 font-medium">{ref.confidence}% confidence</span>
                              </div>
                              <p className="text-gray-700 mt-1">"{ref.excerpt}"</p>
                            </div>
                          ))}
                        </div>
                      )}
                          ...
                          <div className="space-y-1">
                            {message.followUpQuestions.map((question, index) => (
                              <button
                                ...
                                className="block w-full text-left text-xs p-2 hover:bg-gray-100 rounded transition-colors text-blue-600 hover:text-blue-700"
                              >
                                {question}
                              </button>
                            ))}
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="bg-purple-500 hover:bg-purple-600 disabled:bg-gray-300 text-white px-4 py-3 rounded-lg transition-colors"
            >
